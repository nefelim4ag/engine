...
TEST(AndroidExternalViewEmbedder, SubmitFrame) {
  ...
  // ------------------ Third frame ------------------ //
  {
    ...
    SurfaceFrame::SubmitInfo submit_info;
    submit_info.vsync_id = 456;
    surface_frame->set_submit_info(submit_info);

    // The real vsync id is used by the embedder directly.
    EXPECT_CALL(surface_transaction_mock, Begin());
    EXPECT_CALL(surface_transaction_mock, SetVsyncId(456));
    ...
    EXPECT_EQ(submitted_vsync_id, 0);
    // Doesn't resubmit frame.
    auto postpreroll_result = embedder->PostPrerollAction(raster_thread_merger);
    ASSERT_EQ(PostPrerollResult::kSuccess, postpreroll_result);
